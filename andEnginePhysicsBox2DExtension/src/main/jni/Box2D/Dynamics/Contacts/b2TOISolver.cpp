...
void b2TOISolver::Initialize(b2Contact * *contacts, int32
count,
b2Body *toiBody
)
{
...
bool b2TOISolver::Solve(float32 baumgarte) {
    ...
    for (int32 i = 0; i < m_count; ++i) {
        ...
        for (int32 j = 0; j < c->pointCount; ++j) {
            ...
            float32 C = b2Clamp(baumgarte * (separation + b2_linearSlop), -b2_maxLinearCorrection,
                                0.0f);
