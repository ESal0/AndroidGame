...
void b2PrismaticJointDef::Initialize(b2Body *b1, b2Body *b2, const b2Vec2 &anchor,
                                     const b2Vec2 &axis) {
    bodyA = b1;
    bodyB = b2;
...
bool b2PrismaticJoint::SolvePositionConstraints(float32 baumgarte) {
    ...
    if (m_enableLimit) {
        ...
        if (b2Abs(m_upperTranslation - m_lowerTranslation) < 2.0f * b2_linearSlop) {
            ...
            C2 = b2Clamp(translation - m_lowerTranslation + b2_linearSlop, -b2_maxLinearCorrection,
                         0.0f);
...
float32 b2PrismaticJoint::GetJointSpeed() const {
    ...
    float32 speed =
            b2Dot(d, b2Cross(w1, axis)) + b2Dot(axis, v2 + b2Cross(w2, r2) - v1 - b2Cross(w1, r1));
    return speed;
}
